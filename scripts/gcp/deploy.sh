...
API_SECRETS="DATABASE_URL=DATABASE_URL:latest,GOOGLE_AI_API_KEY=GOOGLE_AI_API_KEY:latest,STRIPE_SECRET_KEY=STRIPE_SECRET_KEY:latest,STRIPE_WEBHOOK_SECRET=STRIPE_WEBHOOK_SECRET:latest,INTERNAL_API_SECRET=INTERNAL_API_SECRET:latest,APNS_KEY_ID=APNS_KEY_ID:latest,APNS_TEAM_ID=APNS_TEAM_ID:latest,APNS_PRIVATE_KEY_B64=APNS_PRIVATE_KEY_B64:latest"
PROCESSOR_SECRETS="DATABASE_URL=DATABASE_URL:latest,GOOGLE_AI_API_KEY=GOOGLE_AI_API_KEY:latest,APNS_KEY_ID=APNS_KEY_ID:latest,APNS_TEAM_ID=APNS_TEAM_ID:latest,APNS_PRIVATE_KEY_B64=APNS_PRIVATE_KEY_B64:latest"
...
if gcloud secrets describe ANTHROPIC_API_KEY --project "${PROJECT_ID}" >/dev/null 2>&1; then
  PROCESSOR_SECRETS="${PROCESSOR_SECRETS},ANTHROPIC_API_KEY=ANTHROPIC_API_KEY:latest"
fi
...
gcloud run jobs deploy "${PROCESSOR_JOB}" \
  ...
  --task-timeout 60m \
  --max-retries 1 \
  --set-env-vars "GCP_PROJECT_ID=${PROJECT_ID},GCP_REGION=${REGION},GCP_PROCESSOR_JOB_NAME=${PROCESSOR_JOB},GCS_BUCKET_NAME=${BUCKET},PROCESSOR_MAX_JOBS=25,PROCESSOR_MAX_RUNTIME_SECONDS=3300,APNS_BUNDLE_ID=com.treehomeschool.app" \
