            ...
            <section className="site-panel rounded-[28px] px-6 py-7 sm:px-8">
              ...
              <div className="mt-5 overflow-hidden rounded-[20px] border border-[#e2d2b8]">
                {grades.entries.length === 0 ? <p className="bg-[#fffaf2] px-5 py-8 text-sm text-ink/60">No grade entries for this view yet.</p> : grades.entries.map((entry) => (
                  <div key={`${entry.weeklyPlanId}-${entry.dayNumber ?? "legacy"}-${entry.subjectKey}`} className="flex flex-col gap-3 border-b border-[#eadfcd] bg-white px-5 py-4 last:border-0 sm:flex-row sm:items-center sm:justify-between">
                    <div><div className="flex flex-wrap items-center gap-2"><p className="font-semibold text-ink">Week {entry.weekNumber}{entry.dayNumber ? ` · Day ${entry.dayNumber}` : ""}{grades.selectedSubject ? "" : ` · ${entry.subjectLabel}`}</p>{entry.assessmentRecommended ? <span className="rounded-full bg-[#f3e6c8] px-2 py-1 text-[10px] font-black uppercase tracking-[0.08em] text-[#765632]">Assessment</span> : null}</div><p className="mt-1 text-sm text-ink/58">{entry.planTitle ?? "Subject work"} · updated {formatDate(entry.updatedAt)}</p></div>
                    <div className="flex items-center gap-3"><span className="text-lg font-semibold text-ink">{entry.score == null ? "Not graded" : `${entry.score}%`}</span>{entry.grade ? <span className="rounded-full bg-[#e5efd9] px-3 py-1 text-sm font-semibold text-[#486a38]">{entry.grade}</span> : null}</div>
                  </div>
                ))}
              </div>
            </section>
