...
export default async function ParentCurriculumSubjectDetailPage({
  ...
  const { curriculumSlug, subjectSlug } = params;
  if (!curriculumSlug || !subjectSlug) {
    notFound();
  }
  ...
  let nodes: TreeNode[];
  try {
    nodes = await getParentCurriculumTree(subjectSlug, locale, currentUser.id);
  } catch {
    notFound();
  }
  ...
  const redirectTo = searchParams?.lang
    ? `/p/curriculums/${curriculumSlug}/subjects/${subjectSlug}?lang=${searchParams.lang}`
    : `/p/curriculums/${curriculumSlug}/subjects/${subjectSlug}`;
      ...
      <main className="min-h-screen bg-[#f8f1e4] px-4 py-10 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-6xl">
          <div className="border-b border-[#e2d0b1] pb-6">
            <Link
              href="/"
              className="inline-flex items-center gap-0 text-[28px] font-semibold tracking-[-0.05em] text-ink"
            >
            ...
            <div className="mt-6">
              <Link
                href={`/p/curriculums/${curriculumSlug}/subjects`}
                      ...
                      <div className="mt-6 grid gap-5 lg:grid-cols-2">
                        {clusters.map((cluster) => {
                          ...
                          return (
                            <div
                              key={cluster.id}
                              className="rounded-[24px] border border-[#dcc8aa] bg-[#fffaf2] px-5 py-5"
                            >
                              <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[#6d8f52]">
                                Cluster
                              </p>
                              ...
                              <div className="mt-4 space-y-3">{clusterStandards.map(renderStandard)}</div>
                            </div>
                          );
                        })}
                      </div>
                    ) : (
                      <div className="mt-6 space-y-3">{standards.map(renderStandard)}</div>
                    )}
                  </div>
                );
              })}
              ...
              {topClusters.map((cluster) => {
                ...
                return (
                  <div key={cluster.id} className="site-panel rounded-[28px] px-6 py-7">
                    ...
                    <div className="mt-5 space-y-3">{standards.map(renderStandard)}</div>
                  </div>
                );
              })}
            </section>
