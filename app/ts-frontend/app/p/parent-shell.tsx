...
export async function ParentShell({
  ...
  return (
    <main className="min-h-screen bg-[#f8f1e4] px-4 py-10 sm:px-6 lg:px-8">
      <div className="mx-auto grid max-w-[1400px] gap-6 lg:grid-cols-[280px_minmax(0,1fr)] lg:items-start">
        <aside className="rounded-[24px] border border-[#dec9a9] bg-[#fffaf2] px-4 py-4 lg:sticky lg:top-4 lg:flex lg:min-h-[calc(100vh-5rem)] lg:flex-col">
          ...
          <div className="mt-6">
            <p className="text-center text-sm font-semibold text-earth">
              {dashboard.welcome}: {activeProfile?.firstName ?? currentUserEmail}
            </p>
            <p className="mt-1 text-center text-xs font-semibold uppercase tracking-[0.14em] text-ink/58">
          ...
          <div className="mt-6 space-y-3">
            {resolvedSidebarLinks.map((link) => (
              <Link
                ...
                href={link.href}
                className={`cta-button cta-button--small w-full ${
                  link.tone === "light"
                    ? "cta-button--light"
                    : link.tone === "dark"
                      ? "cta-button--dark"
              ...
              <>
                {workbookNavigation.purchasedWorkbookCount > 0 ? (
                  <Link
                    ...
                    className={`cta-button cta-button--small w-full ${activeNav === "workbooks" ? "cta-button--light" : "cta-button--outline"}`}
                  >
                    Purchased Workbooks
                  </Link>
                ) : null}
                {canManageAccount ? <Link
                    href="/p/settings"
                    className={`cta-button cta-button--small w-full ${activeNav === "settings" ? "cta-button--light" : "cta-button--outline"}`}
                  >
                ...
                {canManageAccount ? <Link
                    href="/p/billing"
                    className={`cta-button cta-button--small w-full ${activeNav === "billing" ? "cta-button--light" : "cta-button--outline"}`}
                  >
